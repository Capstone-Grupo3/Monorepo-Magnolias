...
export default function RankingsView({ rankings }: RankingsViewProps) {
  ...
  const exportarExcel = useCallback(() => {
    ...
    let html = `
      ...
      <table border="1">
        <thead>
          <tr style="background-color: #465fff; color: white; font-weight: bold;">
            <th>Posición</th>
            <th>Candidato</th>
            <th>Correo</th>
            <th>Cargo</th>
    ...
    datosExportar.forEach((rank, index) => {
      const bgColor = index < 3 ? "#eef0ff" : (index % 2 === 0 ? "#ffffff" : "#f9fafb");
      html += `
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="surface-card rounded-xl border border-border-subtle p-5">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <button
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${
                mostrarSelectorEmpresas || empresasSeleccionadas.size > 0
                  ? "border-primary primary-soft text-primary"
                  : "border-border-default surface-muted text-secondary hover:bg-surface-hover"
              }`}
            >
            ...
            <AnimatePresence>
              {mostrarSelectorEmpresas && (
                <motion.div
                  ...
                  className="absolute top-full left-0 mt-2 w-80 surface-card rounded-xl shadow-xl border border-border-subtle z-50 overflow-hidden"
                >
                  <div className="p-3 border-b border-border-subtle surface-muted flex gap-2">
                    <button
                      onClick={seleccionarTodas}
                      className="flex-1 text-sm font-medium text-primary hover:text-primary/80 hover:bg-primary-soft px-3 py-1.5 rounded-lg transition-colors"
                    >
                      Seleccionar todas
                    </button>
                    <button
                      onClick={deseleccionarTodas}
                      className="flex-1 text-sm font-medium text-secondary hover:text-secondary hover:bg-surface-hover px-3 py-1.5 rounded-lg transition-colors"
                    >
                      Limpiar selección
                    </button>
                  </div>
                  ...
                  <div className="max-h-64 overflow-y-auto p-2">
                    {empresasUnicas.length === 0 ? (
                      <p className="text-center text-muted py-4">No hay empresas disponibles</p>
                    ) : (
                      empresasUnicas.map((empresa) => (
                        ...
                        >
                          ...
                          <input
                            ...
                            className="sr-only"
                          />
                          <span className="text-primary dark:text-white font-medium">{empresa}</span>
                  ...
                  <div className="p-3 border-t border-border-subtle surface-muted">
                    <button
                      onClick={() => setMostrarSelectorEmpresas(false)}
                      className="w-full py-2 primary-bg text-white font-medium rounded-lg hover:primary-bg-hover transition-colors"
                    >
                      Aplicar filtros
                    </button>
                  </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-success-soft text-success border border-success/30 rounded-xl hover:bg-success/10 transition-all font-medium disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        {empresasSeleccionadas.size > 0 && (
          <div className="mt-4 flex flex-wrap gap-2">
            {Array.from(empresasSeleccionadas).map((empresa) => (
              <span
                key={empresa}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 primary-soft text-primary rounded-full text-sm font-medium"
              >
      ...
      {rankingsFiltrados.length >= 3 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="surface-card rounded-xl border border-border-subtle p-6 border-t-4 border-gray-400 order-2 md:order-1">
            <div className="flex flex-col items-center text-center">
              <div className="w-16 h-16 surface-muted rounded-full flex items-center justify-center mb-4">
              ...
              <p className="text-sm text-secondary mb-2">{rankingsFiltrados[1].cargo}</p>
              <p className="text-xs text-muted mb-4">{rankingsFiltrados[1].empresa}</p>
              <div className="surface-muted px-4 py-2 rounded-full">
          ...
          <div className="surface-card rounded-xl border border-border-subtle p-6 border-t-4 border-yellow-400 transform md:scale-105 order-1 md:order-2">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-secondary mb-2">{rankingsFiltrados[0].cargo}</p>
              <p className="text-xs text-muted mb-4">{rankingsFiltrados[0].empresa}</p>
          ...
          <div className="surface-card rounded-xl border border-border-subtle p-6 border-t-4 border-orange-400 order-3">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-secondary mb-2">{rankingsFiltrados[2].cargo}</p>
              <p className="text-xs text-muted mb-4">{rankingsFiltrados[2].empresa}</p>
      ...
      <div className="surface-card rounded-xl border border-border-subtle overflow-hidden">
        <div className="px-6 py-4 border-b border-border-subtle">
          <h3 className="text-lg font-semibold flex items-center gap-2 text-primary dark:text-white">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-border-subtle">
            <thead className="surface-muted">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">Posición</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">Candidato</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">Correo</th>
